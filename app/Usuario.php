...
class Usuario extends Authenticatable
{
    ...
    protected $fillable = [
        'usuario', 'password', 'perfil_id', 'remember_token','fecha_registro','last_session'
    ];
    ...
    public static function RevisarSesion($usuario){
        $listar = DB::select(DB::raw(' 
                  select * from users where usuario="'.$usuario.'"'));
         return $listar;
    } 
    public static function ActualizarSesion($usuario,$sesion)
    {
            $listar = DB::select(DB::raw(' 
                  UPDATE  users set last_session='.$sesion.' where usuario="'.$usuario.'"'));
         return $listar;
    }
}
