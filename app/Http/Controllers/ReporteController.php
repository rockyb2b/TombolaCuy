...
class ReporteController extends Controller
{
    ...
    public function CargarDatosCierre()
    {
        $data = "";
        $mensaje = "";
        try {
            // $data_cajaactiva = Reporte::AperturaCajaListarActiva();
            $usuarionombre = Auth::user()->usuario;//"BTD OSCAR AGUILAR";
            $usuario = Auth::user()->idUsuario;
            $data_cajaactiva = AperturaCaja::AperturaCajaListarActiva($usuario)[0];
            $data_cajaactiva->Impreso_en=date('Y-m-d H:i:s');
            $data_cajaactiva->Impreso_por=$usuarionombre;
            $data_cajaactiva->Moneda_valor="Soles";
            $data_cajaactiva->IdInforme=$data_cajaactiva->idAperturaCaja;
            $data_cajaactiva->venta_valor=111;
            $data_cajaactiva->cancelar_valor=222;
            $data_cajaactiva->netoventas_valor=333;
            $data_cajaactiva->saldoneto_valor=333;
        } catch (QueryException $ex) {
            $mensaje = $ex->errorInfo;
        }
        return response()->json(['data' => $data_cajaactiva, 'mensaje' => $mensaje]);
    }
